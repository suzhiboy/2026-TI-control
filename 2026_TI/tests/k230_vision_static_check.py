...
def main():
    ...
    require("MAX_JUMP_MM = 120" in source, "jump rejection must allow full-pipe fast motion")
    require("INIT_CONFIRM_FRAMES = 1" in source, "first valid ball frame must be accepted immediately")
    require("ALLOW_JUMP_RELOCK = True" in source, "jump relock must be enabled")
    require("REINIT_AFTER_LOST_FRAMES = 5" in source, "lost reinitialization must be quick")
    require("JUMP_CONFIRM_FRAMES = 2" in source, "jump confirmation must not add visible delay")
    require("LOST_FRAME_LIMIT = 1" in source, "lost threshold must react in one frame")
    require("TERMINAL_PRINT_EVERY_N_FRAMES = 30" in source, "terminal print must be throttled")
    require("LOST_SEND_EVERY_N_FRAMES = 1" in source, "lost state must be sent every lost frame")
    require("DEBUG_FRAME_STATUS = False" in source, "frame status debug output must be disabled during control")
    ...
    require("post_process_anchor_outputs" in source, "K230 script must post-process AnchorBaseDet output")
    require("aicube.anchorbasedet_post_process" in source, "K230 script must use aicube AnchorBaseDet post-process")
    require("decode_aicube_det_boxes" in source, "K230 script must decode aicube boxes")
    require("decode_canmv_yolo_result" in source, "K230 script must keep CanMV YOLO fallback decoder")
    require("boxes = result[0]" in source, "K230 fallback decoder must read boxes from result[0]")
    require("scores = select_score_list(result, box_count)" in source,
            "K230 fallback decoder must select the valid score list")
    ...
    require("FILTER_ALPHA = 0.85" in source, "low-pass filter alpha must favor real-time motion")
    require("MEDIAN_WINDOW = 1" in source, "median filter must not add frame delay during control")
