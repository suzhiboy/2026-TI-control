...
RAW_RESULT_PRINT_EVERY_N_FRAMES = 30  # Anchor debug; set to 0 for competition.
...
def draw_anchor_results(osd_img, det_boxes, cx, cy, x_mm, fps):
    ...
    if det_boxes:
        for det_box in det_boxes:
            if list_len(det_box) < 6:
                continue
            x1 = ai_to_control_x(det_box[2])
            y1 = ai_to_control_y(det_box[3])
            x2 = ai_to_control_x(det_box[4])
            y2 = ai_to_control_y(det_box[5])
            w = x2 - x1
            h = y2 - y1
    ...
    if cx is not None and cy is not None:
        osd_img.draw_circle(int(cx), int(cy), 3, color=(255, 0, 0), fill=True)
        osd_img.draw_string_advanced(2, 30, 24, "cx=" + str(round_int(cx)), color=(0, 255, 0))
        osd_img.draw_string_advanced(2, 56, 24, "x=" + str(x_mm), color=(255, 0, 0))
...
def main():
    ...
    try:
        ...
        while True:
            ...
            draw_anchor_results(osd_img, det_boxes, cx, cy, x_mm, clock.fps())
            print_frame_status(
                frame_id,
                clock.fps(),
                seen,
