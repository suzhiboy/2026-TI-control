...
# RTSP is intentionally low load. Detection and UART output stay first priority.
WIFI_SSID = "ICE"
WIFI_PASSWORD = "15013623467"
WIFI_CONNECT_TIMEOUT_S = 20
RTSP_PORT = 8554
RTSP_SESSION = "live"
RTSP_CAMERA_CHN = CAM_CHN_ID_1
RTSP_VIDEO_WIDTH = 640
RTSP_VIDEO_HEIGHT = 360
RTSP_VIDEO_BITRATE_KBPS = 600
RTSP_VIDEO_GOP = 15
RTSP_VIDEO_OUTPUT_FPS = 10
RTSP_SENSOR_INPUT_FPS = 30
RTSP_VENC_CHANNEL = VENC_CHN_ID_0
RTSP_VENC_BUFFER_NUM = 8
RTSP_TIMESTAMP = 1000
RTSP_PUMP_EVERY_N_FRAMES = 1
RTSP_ERROR_PRINT_LIMIT = 3
OSD_OVERLAY_AVAILABLE = True
...
def connect_wifi(ssid, password, timeout_s=20):
    ...
    while not wlan.isconnected():
        ...
        if time.ticks_diff(now_ms, last_print_ms) >= 1000:
            print("[wifi] wait {}s".format(time.ticks_diff(now_ms, start_ms) // 1000))
...
def create_h264_channel_attr(encoder, width, height):
    try:
        return ChnAttrStr(
            ...
            width,
            height,
            RTSP_VIDEO_BITRATE_KBPS,
            RTSP_VIDEO_GOP,
            RTSP_SENSOR_INPUT_FPS,
            RTSP_VIDEO_OUTPUT_FPS,
        )
    except TypeError as e:
        print("[rtsp] ChnAttrStr low-load args unsupported: {}".format(e))
        print("[rtsp] fallback to firmware default bitrate/fps")
        return ChnAttrStr(
            encoder.PAYLOAD_TYPE_H264,
            encoder.H264_PROFILE_MAIN,
            width,
...
def prepare_rtsp_encoder(sensor):
    if not ENABLE_RTSP_STREAM:
        return None
    if Encoder is None or mm is None or uctypes is None:
        raise RuntimeError("RTSP/VENC modules are not available")

    width = align_up(RTSP_VIDEO_WIDTH, 16)
    height = RTSP_VIDEO_HEIGHT
    encoder = Encoder()
    encoder.SetOutBufs(RTSP_VENC_CHANNEL, RTSP_VENC_BUFFER_NUM, width, height)

    sensor_source = sensor.bind_info(chn=RTSP_CAMERA_CHN)["src"]
    encoder_destination = (VIDEO_ENCODE_MOD_ID, VENC_DEV_ID, RTSP_VENC_CHANNEL)
...
def start_rtsp_encoder(rtsp_state, wlan):
    if rtsp_state is None:
        return

    encoder = rtsp_state["encoder"]
    width = rtsp_state["width"]
    height = rtsp_state["height"]

    channel_attr = create_h264_channel_attr(encoder, width, height)
    encoder.Create(RTSP_VENC_CHANNEL, channel_attr)
    rtsp_state["encoder_created"] = True

    rtsp_server = mm.rtsp_server()
    rtsp_server.rtspserver_init(RTSP_PORT)
    rtsp_state["rtsp_initialized"] = True
    rtsp_server.rtspserver_createsession(
        RTSP_SESSION,
        mm.multi_media_type.media_h264,
        False,
    )
    rtsp_state["rtsp_session_created"] = True
    rtsp_server.rtspserver_start()
    rtsp_state["rtsp_started"] = True
    rtsp_state["rtsp_server"] = rtsp_server

    encoder.Start(RTSP_VENC_CHANNEL)
    rtsp_state["encoder_started"] = True
    rtsp_state["stream_data"] = StreamData()
    ...
    try:
        ...
        try:
            ...
            pass
        rtsp_url = "rtsp://{}:{}/{}".format(ip, RTSP_PORT, RTSP_SESSION)
    print("[rtsp] open in VLC: {}".format(rtsp_url))
    print(
        "[rtsp] low-load stream {}x{} {}fps {}kbps, detection first".format(
            width,
            height,
            RTSP_VIDEO_OUTPUT_FPS,
            RTSP_VIDEO_BITRATE_KBPS,
...
def pump_rtsp_stream(rtsp_state, frame_id):
    if rtsp_state is None:
        return
    if frame_id % RTSP_PUMP_EVERY_N_FRAMES != 0:
        return
    if not rtsp_state.get("encoder_started", False):
        return

    encoder = rtsp_state["encoder"]
    stream_data = rtsp_state["stream_data"]
    rtsp_server = rtsp_state["rtsp_server"]
    got_stream = False

    try:
        result = encoder.GetStream(RTSP_VENC_CHANNEL, stream_data)
        if result not in (0, None):
            if rtsp_state["errors"] < RTSP_ERROR_PRINT_LIMIT:
                print("[rtsp] GetStream ret={}".format(result))
            rtsp_state["errors"] += 1
            return
        got_stream = True
        ...
        for pack_index in range(stream_data.pack_cnt):
            ...
            if packet_size <= 0:
                continue
            packet_data = bytes(
                uctypes.bytearray_at(stream_data.data[pack_index], packet_size)
            )
        ...
        rtsp_state["sent_frames"] += 1
    except Exception as e:
        if rtsp_state["errors"] < RTSP_ERROR_PRINT_LIMIT:
            print("[rtsp] pump error:", e)
        rtsp_state["errors"] += 1
    finally:
        if got_stream:
            try:
                encoder.ReleaseStream(RTSP_VENC_CHANNEL, stream_data)
            except Exception:
                pass


def safe_stop_rtsp(rtsp_state):
    if rtsp_state is None:
        return

    encoder = rtsp_state.get("encoder", None)
    rtsp_server = rtsp_state.get("rtsp_server", None)

    if rtsp_state.get("encoder_started", False) and encoder is not None:
        try:
            encoder.Stop(RTSP_VENC_CHANNEL)
        except Exception as e:
            print("[rtsp] encoder.Stop fail:", e)

    if rtsp_state.get("encoder_created", False) and encoder is not None:
        try:
            encoder.Destroy(RTSP_VENC_CHANNEL)
        except Exception as e:
            print("[rtsp] encoder.Destroy fail:", e)

    if rtsp_state.get("rtsp_started", False) and rtsp_server is not None:
        try:
            rtsp_server.rtspserver_stop()
        except Exception as e:
            print("[rtsp] stop fail:", e)

    if rtsp_state.get("rtsp_session_created", False) and rtsp_server is not None:
        try:
            rtsp_server.rtspserver_destroysession(RTSP_SESSION)
        except Exception:
            pass

    if rtsp_state.get("rtsp_initialized", False) and rtsp_server is not None:
        try:
            rtsp_server.rtspserver_deinit()
        except Exception as e:
            print("[rtsp] deinit fail:", e)

    rtsp_state["media_link"] = None


def safe_disconnect_wifi(wlan):
    if wlan is None:
        return
    try:
        if wlan.isconnected():
...
def setup_sensor_and_display():
    ...
    if ENABLE_RTSP_STREAM:
        ...
        sensor.set_pixformat(PIXEL_FORMAT_YUV_SEMIPLANAR_420, chn=RTSP_CAMERA_CHN)
...
def draw_anchor_results(osd_img, selected_box, cx, cy, x_mm, fps):
    if not SHOW_IMAGE or osd_img is None:
        return
    global OSD_OVERLAY_AVAILABLE
    if not OSD_OVERLAY_ENABLED or not OSD_OVERLAY_AVAILABLE:
        return
    ...
    try:
        ...
        OSD_OVERLAY_AVAILABLE = False
        print("[display] OSD overlay disabled:", e)
...
def main():
    ...
    try:
        ...
        if ENABLE_RTSP_STREAM:
            try:
                wlan = connect_wifi(WIFI_SSID, WIFI_PASSWORD, WIFI_CONNECT_TIMEOUT_S)
                rtsp_state = prepare_rtsp_encoder(sensor)
            except Exception as e:
                rtsp_state = None
                print("[rtsp] disabled before MediaManager.init:", e)

        MediaManager.init()
        if rtsp_state is not None:
            try:
                start_rtsp_encoder(rtsp_state, wlan)
            except Exception as e:
                print("[rtsp] disabled after MediaManager.init:", e)
                safe_stop_rtsp(rtsp_state)
                rtsp_state = None
        ...
        while True:
            ...
            pump_rtsp_stream(rtsp_state, frame_id)
